...
local Library = {
    ...
    SwitchTheme = function(self, themeName)
        if self.Themes[themeName] then
            self.CurrentTheme = themeName
            for category, color in pairs(self.Themes[themeName]) do
                for _, obj in pairs(self.ThemeObjects[category]) do
    ...
    SaveSettings = function(self)
        local settings = {
            ...
            DragSpeed = self.DragSpeed,
            LockDragging = self.LockDragging
        }
        writefile("LibrarySettings.json", game:GetService("HttpService"):JSONEncode(settings))
    end,

    -- Function to load settings
    LoadSettings = function(self)
        if isfile("LibrarySettings.json") then
            local settings = game:GetService("HttpService"):JSONDecode(readfile("LibrarySettings.json"))
            self:SwitchTheme(settings.Theme)
            self.DragSpeed = settings.DragSpeed
            self.LockDragging = settings.LockDragging
        end
    end
}

-- Example usage
Library:SwitchTheme("Aqua")
...
function Library:create_gradient(element, color1, color2)
...
exampleFrame.Size = UDim2.new(0, 200, 0, 100)
exampleFrame.Position = UDim2.new(0.5, -100, 0.5, -50)
exampleFrame.Parent = game.Players.LocalPlayer:WaitForChild("PlayerGui"):WaitForChild("ScreenGui")
Library:create_gradient(exampleFrame, Color3.fromRGB(255, 0, 0), Color3.fromRGB(0, 0, 255))
